...
import { getActiveCategories, getActiveProducts, getProductImages, getPublicImageUrl } from '../lib/catalogue'
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        setCats(c)
        setProducts(p)
        // Pre-check images for each product (isolated, will fallback to placeholder if bucket missing)
        ...
        if (!cancelled) setImageMap(entries)
      } catch (e) {
          ...
          {filtered.map(p => {
            ...
            return (
              <Link key={p.id} to={`/product/${p.id}`} className="bg-white border border-zinc-200 rounded-2xl p-4 hover:border-zinc-300 flex flex-col gap-2">
                <div className="text-xs text-zinc-500 uppercase tracking-wide">{cats.find(c => c.id === p.category_id)?.name ?? 'Uncategorized'} · {p.product_type}</div>
                <div className="font-semibold line-clamp-2">{p.name}</div>
                <div className="text-sm font-bold">
                  {p.sale_price != null ? `GH₵ ${p.sale_price}` : p.base_price != null ? `GH₵ ${p.base_price}` : '—'}
                  {p.sale_price != null && p.base_price != null && p.sale_price < p.base_price && <span className="ml-2 text-xs font-normal line-through text-zinc-400">GH₵ {p.base_price}</span>}
                </div>
                <div className="text-xs text-zinc-400">SKU {p.sku ?? '—'}</div>
                {hasImage ? (
                  <div className="mt-2 text-xs bg-green-50 border border-green-200 rounded-xl p-3 text-green-700">Image available (public URL)</div>
                ) : (
